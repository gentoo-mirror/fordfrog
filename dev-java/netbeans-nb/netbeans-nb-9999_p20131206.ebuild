...
src_install() {
	...
	# change to oracle jdk because of javafx not being present in other jdks
	sed -i 's%netbeans_jdkhome="\$(java-config -O)"%netbeans_jdkhome="/usr/lib/jvm/oracle-jdk-bin-1.7/"%g' "${D}"/etc/netbeans-${SLOT}/netbeans.conf || die
