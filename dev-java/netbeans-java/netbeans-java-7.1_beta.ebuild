...
SLOT="7.1"
SOURCE_URL="http://dlc.sun.com.edgesuite.net/netbeans/7.1/beta/zip/netbeans-7.1beta-201109252201-src.zip"
SRC_URI="${SOURCE_URL}
	http://dev.gentoo.org/~fordfrog/distfiles/netbeans-7.1-build.xml.patch.bz2
...
src_unpack() {
	...
	unpack netbeans-7.1-build.xml.patch.bz2
...
src_prepare() {
	...
	epatch netbeans-7.1-build.xml.patch

	# Support for custom patches
	if [ -n "${NETBEANS71_PATCHES_DIR}" -a -d "${NETBEANS71_PATCHES_DIR}" ] ; then
		local files=`find "${NETBEANS71_PATCHES_DIR}" -type f`
